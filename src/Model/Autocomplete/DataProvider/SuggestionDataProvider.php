...
class SuggestionDataProvider implements DataProviderInterface
{
    ...
    private function processResponse($response): array
    {
        ...
        if ($response instanceof SuggestionsResponse) { // @phpstan-ignore-line
    ...
    protected function getSuggestionBlocks(AutocompleteProductResponseInterface $response): array
    {
        ...
        $blocks = $response->getBlocks() ? $response->getBlocks() : []; // @phpstan-ignore-line
        if (empty($blocks)) {
            return $results;
        }
        ...
        foreach ($products as $product) {
            $productItemsByTweakwiseId[(string)$product->getProduct()->getData('tweakwise_id')] = $product->toArray(); // @phpstan-ignore-line
        }
