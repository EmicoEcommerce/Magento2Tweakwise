...
class TweakwiseCheckout implements ObserverInterface
{
    ...
    {
        if (empty($items) && !is_array($items)) { // @phpstan-ignore-line
            return;
        }
        ...
        if ($this->config->isGroupedProductsEnabled()) {
            ...
            foreach ($items as $originalItem) {
                ...
                $returnedItem->setData('groupCode', $originalItem->getProductId()); // @phpstan-ignore-line
                $filteredItems[(int)$returnedItem->getId()] = $returnedItem; // @phpstan-ignore-line
            }
        ...
        foreach ($items as $item) {
            if ($this->config->isGroupedProductsEnabled()) {
                $originalItem = $item->getData('groupCode'); // @phpstan-ignore-line
